...
docker-build:
	docker build -f docker/real/Dockerfile -t $(OPERATOR_IMAGE):$(TAG) -t $(OPERATOR_IMAGE):latest $(PWD)
...
docker-shell:
	docker run -it --entrypoint=/bin/bash usgseros/$(OPERATOR_IMAGE):latest
